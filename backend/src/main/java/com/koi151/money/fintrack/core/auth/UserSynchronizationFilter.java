...
public class UserSynchronizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        if (authentication instanceof JwtAuthenticationToken jwtAuth) {
            ...
            if (subject != null) {
                ...
                // Cache look up: Get the specific cache region for User Sync
                Cache userSyncCache = cacheManager.getCache(CacheRegion.USER_SYNC.getCacheName());

                // Sync only if not present in cache
                if (userSyncCache != null && !isUserSynchronized(userSyncCache, userId)) {
                    log.debug("User {} missing in sync cache. Triggering async synchronization.", userId);
    ...
    */
    ...
    /**
    * Update cache, marks the user as synchronized in the cache to prevent redundant DB checks.
    */
